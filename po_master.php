<?php
// Jagdamba Electrical - PO Master (po_master.php)
...
<div class="modal-backdrop" id="import-modal">
    <div class="modal-content" style="width: 700px; max-width: 95%;">
        ...
        <div class="modal-body">
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                <label class="form-label" for="import-excel-date">Default PO Date (Applies to all imported rows)</label>
                <input type="date" id="import-excel-date" class="form-control" style="background-color: var(--bg-main); color: var(--text-main);">
            </div>
...
<script>
    ...
    function closeImportModal() {
        ...
        document.getElementById('import-excel-date').value = '';
    ...
    function parseExcelDate(val) {
        ...
        // If it's a numeric string (e.g. "46118"), convert it to a number first
        ...
        // If it's a string, try splitting any time portion and pattern matching first
        if (typeof val === 'string') {
            const datePart = val.split(/[ T]/)[0];
            const parsedStr = parseDateString(datePart);
            if (parsedStr) return parsedStr;
        }

        // If it's already a Date object (SheetJS cellDates: true)
        if (val instanceof Date) {
            // Adjust for local timezone offset to get local time milliseconds
            const localMs = val.getTime() - (val.getTimezoneOffset() * 60 * 1000);
            // Round to nearest day
            const roundedDate = new Date(Math.round(localMs / 86400000) * 86400000);
            const yyyy = roundedDate.getUTCFullYear();
            const mm = String(roundedDate.getUTCMonth() + 1).padStart(2, '0');
            const dd = String(roundedDate.getUTCDate()).padStart(2, '0');
            return `${yyyy}-${mm}-${dd}`;
        }
    ...
    async function executeBulkImport() {
        ...
        const defaultDateVal = document.getElementById('import-excel-date').value || null;
