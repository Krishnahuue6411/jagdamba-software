<?php
// Jagdamba Electrical - System Aging Alerts (alerts.php)
...
<div class="stats-grid" style="margin-bottom: 24px;">
    <div class="stat-card">
        <div class="stat-info">
            <span class="stat-label">Aged Stock Batches</span>
...
<div class="panel" style="margin-bottom: 24px;">
    <div class="panel-header" style="background-color: rgba(255, 255, 255, 0.01); border-bottom: 1px solid var(--border-color);">
        ...
        <span class="panel-subtitle">Narrow down alerts by dates, age threshold, keyword, or severity</span>
    </div>
    <div class="panel-body" style="padding: 24px;">
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 16px; align-items: flex-end;">
            ...
            <div class="form-group">
                <label class="form-label" for="filter-min-days">Min Age (Days)</label>
                <select id="filter-min-days" class="form-control" onchange="applyFilters()" style="height: 42px; background: var(--bg-main); border: 1px solid var(--border-color); color: var(--text-main); border-radius: 8px; padding: 0 12px; font-family: var(--font-body); font-weight: 500;">
            ...
            <!-- Age Severity Bracket -->
            <div class="form-group">
                <label class="form-label" for="filter-age-group">Severity</label>
                <select id="filter-age-group" class="form-control" onchange="applyFilters()" style="height: 42px; background: var(--bg-main); border: 1px solid var(--border-color); color: var(--text-main); border-radius: 8px; padding: 0 12px; font-family: var(--font-body); font-weight: 500;">
                    <option value="all">All Aged</option>
            ...
            <div class="form-group" style="min-width: 180px;">
                ...
                <div class="topbar-search" style="display: flex; width: 100%;">
                    ...
                    <input type="text" id="filter-search" placeholder="Material Code, Challan, Type..." style="width: 100%; padding-left: 40px; height: 42px;" oninput="applyFilters()">
                </div>
            </div>
...
<div class="panel">
    <div class="panel-header" style="flex-wrap: wrap; gap: 12px; border-bottom: 1px solid var(--border-color);">
        ...
        <div style="display: flex; align-items: center; gap: 16px;">
            ...
            <span id="active-threshold-label" style="font-size: 0.8rem; background: rgba(245,158,11,0.12); color: var(--warning); border: 1px solid rgba(245,158,11,0.25); border-radius: 20px; padding: 2px 12px; font-weight: 600;"></span>
...
<script>
    ...
    // Load ALL active stock (no hardcoded day limit — filtering done client-side)
    async function loadAlertsData() {
        App.showLoading('Retrieving inventory records...');
        try {
            const res = await App.api('query', {
                sql: "SELECT id, ch_no, ch_date, m_code, m_description, material_type, in_qty, bal_qty, unit, party_id, DATEDIFF(CURDATE(), ch_date) as age_days FROM inward_transaction WHERE bal_qty > 0 ORDER BY age_days DESC"
            });
    ...
    function applyFilters() {
        const minDays   = parseInt(document.getElementById('filter-min-days').value || 30);
        ...
        // Update the active threshold pill
        document.getElementById('active-threshold-label').innerText = `Showing ≥ ${minDays} Days`;
        ...
        filteredAlerts = alertsCache.filter(item => {
            ...
            // 1. Minimum age threshold filter
            if (age < minDays) return false;

            // 2. Severity bracket filter
            if (ageGroup === 'warning'  && (age <= 30 || age > 60)) return false;
            ...
            if (searchVal) {
                ...
                const mc   = (item.m_code         || '').toLowerCase();
                const desc = (item.m_description  || '').toLowerCase();
                const type = (item.material_type  || '').toLowerCase();
                return ch.includes(searchVal) || mc.includes(searchVal) || desc.includes(searchVal) || type.includes(searchVal);
            }
    ...
    function populateGrid(data) {
        ...
        data.forEach(item => {
            ...
            if (age > 60) {
                ...
                statusBadge = '<span class="badge" style="font-weight:700; background-color:rgba(16,185,129,0.15); color:var(--success); padding:4px 8px; border-radius:4px; font-size:0.75rem;">Recent</span>';
                ageColor    = 'color: var(--success); font-weight: 600;';
            }
            ...
            const mDesc = item.m_description ? escapeHtml(item.m_description) : `<span style="color:var(--text-muted); font-style:italic;">N/A</span>`;
    ...
    function updateStatistics(data) {
        ...
        let totalQty = 0;
        data.forEach(item => { totalQty += parseFloat(item.bal_qty || 0); });
        document.getElementById('stats-total-qty').innerText = App.formatDecimal(totalQty);
    }

    // Reset all filters to defaults
    function resetFilters() {
        document.getElementById('filter-min-days').value   = '30';
    ...
    function exportToExcel() {
        if (filteredAlerts.length === 0) { alert("No records found to export."); return; }
        ...
        const data = filteredAlerts.map(item => ({
            ...
            "Unit":                  item.unit || 'NOS',
            "Severity Status":       parseInt(item.age_days || 0) > 60 ? "Critical" : parseInt(item.age_days || 0) > 30 ? "Warning" : "Recent"
        }));
        ...
        worksheet['!cols'] = [
            ...
            { wch: 25 }, { wch: 15 }, { wch: 18 }, { wch: 18 }, { wch: 10 }, { wch: 15 }
        ];
        XLSX.writeFile(workbook, `Aged_Stock_Alerts_min${minDays}days.xlsx`);
    }
    ...
    function exportToPDF() {
        ...
        doc.text(`Aged Stock Alerts Registry (Stock older than ${minDays} Days)`, 14, 26);
        ...
        const rows = filteredAlerts.map(item => ({
            ch_date:       item.ch_date ? App.formatDate(item.ch_date) : 'N/A',
            ...
            severity:      parseInt(item.age_days || 0) > 60 ? "Critical" : parseInt(item.age_days || 0) > 30 ? "Warning" : "Recent"
        }));

        doc.autoTable({
            columns: columns, body: rows, startY: 37, theme: 'striped',
            headStyles: { fillColor: [37, 99, 235], textColor: [255, 255, 255], fontStyle: 'bold' },
            alternateRowStyles: { fillColor: [245, 245, 245] },
            columnStyles: {
                age_days: { halign: 'right' },
                in_qty:   { halign: 'right' },
                bal_qty:  { halign: 'right', fontStyle: 'bold', textColor: [37, 99, 235] }
            },
            didParseCell: function(data) {
                if (data.column.key === 'severity') {
                    ...
                    else if (data.cell.raw === "Warning") { data.cell.styles.textColor = [245, 158, 11]; data.cell.styles.fontStyle = 'bold'; }
                }
            }
        });

        doc.save(`Aged_Stock_Alerts_min${minDays}days.pdf`);
    }
