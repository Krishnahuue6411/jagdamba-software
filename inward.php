<?php
// Jagdamba Electrical - Inward Transaction Entry (inward.php)
...
<script>
    ...
    function populateGrid(data) {
        ...
        if (data.length === 0) {
            body.innerHTML = '<tr><td colspan="10" style="text-align:center; color:var(--text-muted); padding:32px;">No inward records recorded.</td></tr>';
            return;
        }
        ...
        data.forEach(item => {
            ...
            const canTransfer = parseFloat(item.bal_qty) > 0;
            const transferBtn = canTransfer
                ? `<button class="btn" style="background:var(--pending); color:white; border:none; padding:6px 10px; border-radius:4px; font-size:0.7rem; font-weight:bold; cursor:pointer; display:inline-flex; align-items:center; gap:4px;" onclick="event.stopPropagation(); transferToChallan(${item.id})" title="Transfer to Challan">
                      <i class="fa-solid fa-right-left"></i> Transfer to Challan
                   </button>`
                : `<button class="btn" style="background:rgba(255,255,255,0.05); color:var(--text-muted); border:1px solid var(--border-color); padding:6px 10px; border-radius:4px; font-size:0.7rem; font-weight:bold; cursor:not-allowed; display:inline-flex; align-items:center; gap:4px;" disabled title="Fully Consumed">
                      <i class="fa-solid fa-circle-minus"></i> Consumed
                   </button>`;
            ...
            tr.innerHTML = `
                ...
                <td style="text-align: center; vertical-align: middle;">${transferBtn}</td>
            `;
            body.appendChild(tr);
        });
    }

    // Call API to transfer inward to challan inward
    async function transferToChallan(id) {
        const item = inwardsList.find(x => x.id == id);
        if (!item) return;

        const balQty = parseFloat(item.bal_qty || 0);
        if (balQty <= 0) {
            App.showToast('This item has already been fully consumed and cannot be transferred.', 'error');
            return;
        }

        const inputVal = prompt(`Enter quantity to transfer for material "${item.m_code}" (Challan "${item.ch_no}") to Challan Inward:\n(Available Balance: ${App.formatDecimal(balQty)} ${item.unit || 'units'})`, balQty);
        ...
        App.showLoading('Transferring to Challan Inward...');
        try {
            const res = await App.api('transfer_inward_to_challan', { id: parseInt(id), transfer_qty: transferQty });
            if (res.ok) {
                App.showToast(`Successfully transferred ${App.formatDecimal(transferQty)} ${item.unit || 'units'} to Challan Inward!`, 'success');
