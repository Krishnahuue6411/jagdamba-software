<?php
// Jagdamba Electrical - Tax Invoice Module (tax_invoice.php)
                ...
                <div class="form-grid">
                    ...
                    <div class="form-grid two-col" style="margin-bottom: 16px;">
                        <div class="form-group">
                            ...
                            <input type="text" id="inv_no" class="form-control" placeholder="Auto-filled from last invoice" required autocomplete="off" onblur="checkDuplicateInvoiceNo()">
                            <div id="inv-no-feedback" style="font-size:0.78rem; margin-top:4px; min-height:18px;"></div>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-grid two-col" style="margin-bottom: 24px;">
                        <div class="form-group">
                            <label class="form-label" for="asn_no">ASN Number</label>
                            <input type="text" id="asn_no" class="form-control" placeholder="Enter ASN Number" value="9988">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-grid two-col" style="margin-bottom: 24px;">
                        <div class="form-group" style="display: flex; flex-direction: column; justify-content: flex-end;">
                            <label class="form-label" for="show_asn" style="display: flex; align-items: center; gap: 8px; cursor: pointer; margin-bottom: 8px; user-select: none;">
                                <input type="checkbox" id="show_asn" style="width: 18px; height: 18px; cursor: pointer; accent-color: var(--primary);">
                                <span style="font-weight: 600;">Show ASN in Invoice</span>
                            </label>
                        </div>
                        <div class="form-group" style="display: flex; flex-direction: column; justify-content: flex-end;">
                    ...
                    <fieldset id="grand-totals-fieldset" style="display: none; border: 1.5px solid var(--primary); border-radius: 8px; padding: 14px; margin-bottom: 15px; background: rgba(37, 99, 235, 0.03);">
...
<script>
    ...
    async function fetchNextInvoiceNo() {
        ...
        try {
            const res = await App.api('get_next_invoice_no', { party_id: partyId });
            if (res.ok && res.next_no) {
                document.getElementById('inv_no').value = res.next_no;
                setInvNoFeedback('', ''); // clear feedback on auto-fill
            }
        } catch (err) {
    ...
    // Real-time duplicate invoice number check (called on blur of inv_no)
    async function checkDuplicateInvoiceNo() {
        const invNoInput = document.getElementById('inv_no');
        const feedback   = document.getElementById('inv-no-feedback');
        ...
        // In edit mode: if the user keeps the same original number, it's fine
        if (isEditMode && val === originalInvoiceNo) {
            setInvNoFeedback('✓ Current invoice number (edit mode)', 'success');
            return;
        }

        try {
            const res = await App.api('query', {
                sql: "SELECT COUNT(*) as cnt FROM `tax_invoice` WHERE `inv_no` = :inv_no",
                params: { ':inv_no': val }
            });
            const cnt = parseInt(res.rows?.[0]?.cnt || 0);
            if (cnt > 0) {
                setInvNoFeedback(`✗ "${val}" already exists — choose a different number.`, 'error');
            } else {
                setInvNoFeedback(`✓ "${val}" is available`, 'success');
            }
        } catch (err) {
            // Silently ignore (network error etc.)
        }
    }

    function setInvNoFeedback(msg, type) {
        const invNoInput = document.getElementById('inv_no');
        const feedback   = document.getElementById('inv-no-feedback');
        if (!feedback) return;
        if (!msg) {
            feedback.innerHTML = '';
            invNoInput.style.borderColor = '';
            return;
        }
        if (type === 'error') {
            feedback.innerHTML = `<span style="color:var(--error); font-weight:600;">${msg}</span>`;
            invNoInput.style.borderColor = 'var(--error)';
            invNoInput.style.boxShadow = '0 0 0 2px rgba(239,68,68,0.15)';
        } else {
            feedback.innerHTML = `<span style="color:var(--success); font-weight:600;">${msg}</span>`;
            invNoInput.style.borderColor = 'var(--success)';
            invNoInput.style.boxShadow = '0 0 0 2px rgba(16,185,129,0.15)';
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        // Real-time invoice number duplicate check on blur
        document.getElementById('inv_no').addEventListener('blur', () => checkDuplicateInvoiceNo());
    ...
    async function loadInvoiceForEditing(invNo) {
        ...
        try {
            ...
            document.getElementById('show_asn').checked = (header.show_asn !== 0 && header.show_asn !== '0');
    ...
    function handlePartyChange() {
        ...
        syncWoundCodeToDrawingNumber();
        loadPendingMachines(); // Reload machine list for selected party
        fetchNextInvoiceNo();  // Fetch party-specific next invoice number
    }
    ...
    function renderPendingMachinesCheckboxGrid() {
        ...
        Object.values(stdGroups).forEach(group => {
            ...
            label.style.border = '1px dashed rgba(37, 99, 235, 0.3)';
    ...
    function resetForm() {
        ...
        document.getElementById('show_asn').checked = false;
        document.getElementById('inv_date').value = App.formatDate(new Date());
    ...
    async function handleInvoiceSubmit(e) {
        ...
        const show_asn = document.getElementById('show_asn').checked ? 1 : 0;
    ...
    async function validateInvoiceInputs() {
        ...
        async function fetchBomSetsForWoundAndMac(wCode, pNo) {
            if (!wCode) return { set1: [], set2: [] };

            let rows = [];
            if (pNo) {
                const resB = await App.api('query', {
                    sql: "SELECT `rm_code`, `req_qty`, `mac_no`, `bom_tier` FROM `bom` WHERE `wound_code` = :wound_code AND `mac_no` = :part_no ORDER BY `id` ASC",
                    params: { ':wound_code': wCode, ':part_no': pNo }
                });
                rows = resB.rows || [];

                if (rows.length === 0) {
                    const resLike = await App.api('query', {
                        sql: "SELECT `rm_code`, `req_qty`, `mac_no`, `bom_tier` FROM `bom` WHERE `wound_code` = :wound_code AND `mac_no` LIKE :part_like ORDER BY `id` ASC",
                        params: { ':wound_code': wCode, ':part_like': pNo + '%' }
                    });
                    let likeRows = resLike.rows || [];
                    if (likeRows.length > 0) {
                        const firstMac = likeRows[0].mac_no;
                        rows = likeRows.filter(r => r.mac_no === firstMac);
                    }
                }
            }

            if (rows.length > 0) {
                let set1 = rows.filter(b => (b.bom_tier || 'PRIMARY').toUpperCase() !== 'SECONDARY');
                let set2 = rows.filter(b => (b.bom_tier || 'PRIMARY').toUpperCase() === 'SECONDARY');
                if (set2.length === 0) {
                    const resOther = await App.api('query', {
                        sql: "SELECT `rm_code`, `req_qty`, `mac_no`, `bom_tier` FROM `bom` WHERE `wound_code` = :wound_code AND `mac_no` != :mac_no ORDER BY `id` ASC",
                        params: { ':wound_code': wCode, ':mac_no': rows[0].mac_no }
                    });
                    set2 = resOther.rows || [];
                }
                return { set1, set2 };
            ...
            const resAll = await App.api('query', {
                sql: "SELECT `rm_code`, `req_qty`, `mac_no`, `bom_tier` FROM `bom` WHERE `wound_code` = :wound_code ORDER BY `id` ASC",
                params: { ':wound_code': wCode }
            });
            const allRows = resAll.rows || [];
            if (allRows.length === 0) return { set1: [], set2: [] };

            const hasSecondaryTier = allRows.some(b => (b.bom_tier || 'PRIMARY').toUpperCase() === 'SECONDARY');
            if (hasSecondaryTier) {
                const set1 = allRows.filter(b => (b.bom_tier || 'PRIMARY').toUpperCase() !== 'SECONDARY');
                const set2 = allRows.filter(b => (b.bom_tier || 'PRIMARY').toUpperCase() === 'SECONDARY');
                return { set1, set2 };
            }

            const macGroups = {};
            allRows.forEach(r => {
                const key = r.mac_no || 'DEFAULT';
                if (!macGroups[key]) macGroups[key] = [];
                macGroups[key].push(r);
            });
            const keys = Object.keys(macGroups);
            const set1 = macGroups[keys[0]] || [];
            const set2 = keys.length > 1 ? (macGroups[keys[1]] || []) : [];
            return { set1, set2 };
        }

        async function calculateTieredRequirementsJS(bomSets, itemQty, pNo) {
            if (!bomSets || itemQty <= 0) return {};

            const set1 = bomSets.set1 || [];
            const set2 = bomSets.set2 || [];

            const set1RmMap = {};
            set1.forEach(b => {
                if (!set1RmMap[b.rm_code]) set1RmMap[b.rm_code] = parseFloat(b.req_qty);
            });

            const set2RmMap = {};
            set2.forEach(b => {
                if (!set2RmMap[b.rm_code]) set2RmMap[b.rm_code] = parseFloat(b.req_qty);
            });

            const reqs = {};
            if (Object.keys(set2RmMap).length === 0) {
                Object.keys(set1RmMap).forEach(rmCode => {
                    reqs[rmCode] = set1RmMap[rmCode] * itemQty;
                });
                return reqs;
            }

            let q1Max = itemQty;
            for (const rmCode of Object.keys(set1RmMap)) {
                const reqPerUnit = set1RmMap[rmCode];
                if (reqPerUnit > 0) {
                    let avail = 0;
                    if (pNo) {
                        const resStockPart = await App.api('query', {
                            sql: "SELECT COALESCE(SUM(bal_qty), 0) as available_bal FROM inward_transaction WHERE m_code = :m_code AND part_no = :part_no AND bal_qty > 0",
                            params: { ':m_code': rmCode, ':part_no': pNo }
                        });
                        avail = parseFloat((resStockPart.rows && resStockPart.rows[0]) ? resStockPart.rows[0].available_bal : 0);
                    }
                    if (!pNo || avail < reqPerUnit) {
                        const resStockGen = await App.api('query', {
                            sql: "SELECT COALESCE(SUM(bal_qty), 0) as available_bal FROM inward_transaction WHERE m_code = :m_code AND bal_qty > 0",
                            params: { ':m_code': rmCode }
                        });
                        avail = parseFloat((resStockGen.rows && resStockGen.rows[0]) ? resStockGen.rows[0].available_bal : 0);
                    }
                    const possible = Math.floor(avail / reqPerUnit);
                    if (possible < q1Max) q1Max = possible;
                }
            }

            const q1 = Math.max(0, q1Max);
            const q2 = itemQty - q1;

            Object.keys(set1RmMap).forEach(rmCode => {
                reqs[rmCode] = (reqs[rmCode] || 0) + (set1RmMap[rmCode] * q1);
            });
            if (q2 > 0) {
                Object.keys(set2RmMap).forEach(rmCode => {
                    reqs[rmCode] = (reqs[rmCode] || 0) + (set2RmMap[rmCode] * q2);
                });
            }
            return reqs;
        }
        ...
        for (const item of addedItems) {
            try {
                const bomSets = await fetchBomSetsForWoundAndMac(item.wound_code, partNo);
                const subReqs = await calculateTieredRequirementsJS(bomSets, parseFloat(item.qty), partNo);
                Object.keys(subReqs).forEach(rmCode => {
                    requirementsMap[rmCode] = (requirementsMap[rmCode] || 0) + subReqs[rmCode];
                });
            } catch (err) {
                console.error(err);
            }
        ...
        if (qty > 0) {
            try {
                const bomSets = await fetchBomSetsForWoundAndMac(woundCode, partNo);
                const subReqs = await calculateTieredRequirementsJS(bomSets, qty, partNo);
                Object.keys(subReqs).forEach(rmCode => {
                    requirementsMap[rmCode] = (requirementsMap[rmCode] || 0) + subReqs[rmCode];
                });
            } catch (err) {
        ...
        if (isEditMode && originalInvoiceItems.length > 0) {
            for (const item of originalInvoiceItems) {
                try {
                    ...
                    const subReqs = await calculateTieredRequirementsJS(bomSets, parseFloat(item.qty), partNo);
                    Object.keys(subReqs).forEach(rmCode => {
                        originalReqMap[rmCode] = (originalReqMap[rmCode] || 0) + subReqs[rmCode];
                    });
                } catch (err) {
                    console.error(err);
                }
            ...
            try {
                ...
                for (const rmCode of rmCodes) {
                    ...
                    if (partNo) {
                        const resStockPart = await App.api('query', {
                            sql: "SELECT COALESCE(SUM(bal_qty), 0) as available_bal FROM inward_transaction WHERE m_code = :m_code AND part_no = :part_no AND bal_qty > 0",
                            params: { ':m_code': rmCode, ':part_no': partNo }
                        });
                        available = parseFloat(resStockPart.rows[0].available_bal || 0);
                    }

                    // Fallback to overall stock across all part numbers if part_no stock is insufficient
                    if (!partNo || available < totalRequired) {
                        const resStockGen = await App.api('query', {
                            sql: "SELECT COALESCE(SUM(bal_qty), 0) as available_bal FROM inward_transaction WHERE m_code = :m_code AND bal_qty > 0",
                            params: { ':m_code': rmCode }
                        });
                        available = parseFloat(resStockGen.rows[0].available_bal || 0);
                    }
