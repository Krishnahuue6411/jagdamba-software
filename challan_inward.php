<?php
// Jagdamba Electrical - Challan Inward (Non-Payment) Entry (challan_inward.php)
...
<script>
    ...
    function renderGrid(data) {
        ...
        if (data.length === 0) {
            body.innerHTML = '<tr><td colspan="9" style="text-align:center; color:var(--text-muted); padding:32px;">No challan inward records found.</td></tr>';
            return;
        }

        data.forEach(item => {
            const balQty = parseFloat(item.bal_qty ?? item.qty);
            const canTransfer = balQty > 0;
            const transferBtn = canTransfer
                ? `<button class="btn" style="background:var(--pending); color:white; border:none; padding:4px 8px; border-radius:4px; font-size:0.7rem; font-weight:bold; cursor:pointer; display:inline-flex; align-items:center; gap:4px;" onclick="transferChallanToInward(${item.id})" title="Transfer to Inward Entry">
                      <i class="fa-solid fa-right-left"></i> Transfer to Inward
                   </button>`
                : `<span style="font-size:0.75rem; color:var(--text-muted);">Transferred/Consumed</span>`;
    ...
    async function transferChallanToInward(id) {
        const item = recordsCache.find(x => x.id == id);
        if (!item) return;
