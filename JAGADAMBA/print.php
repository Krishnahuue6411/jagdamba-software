<?php
// Sahara Electrical - Print Invoice Template (print.php)
...
if (!$inv_no) {
    $error = "Invoice Number parameter is missing.";
} else {
    try {
        $pdo = getDatabaseConnection();


        // 1. Fetch Invoice Details (multiple items supported)
        $stmt = $pdo->prepare("SELECT * FROM `tax_invoice` WHERE `inv_no` = :inv_no ORDER BY id ASC");
        $stmt->execute([':inv_no' => $inv_no]);
        $invoiceRows = $stmt->fetchAll();

        if (empty($invoiceRows)) {
            $error = "Invoice not found in the database: " . htmlspecialchars($inv_no);
        } else {
            $invoice = $invoiceRows[0];

            // 2. Fetch linked machine numbers
            $stmtMc = $pdo->prepare("SELECT `machine_no` FROM `invoice_machines` WHERE `inv_no` = :inv_no");
            $stmtMc->execute([':inv_no' => $inv_no]);
            while ($row = $stmtMc->fetch()) {
                $machines[] = $row['machine_no'];
            }

            // 3. Fetch transporter name from vehicle number if possible
            $stmtTr = $pdo->prepare("SELECT `transport_name` FROM `transport_master` WHERE `vech_no` = :vehicle_no LIMIT 1");
            $stmtTr->execute([':vehicle_no' => $invoice['vehicle_no']]);
            $trRow = $stmtTr->fetch();
            $transporter = $trRow ? $trRow['transport_name'] : 'By Road';

            // 4. Fetch distinct challan numbers linked to this invoice's machines
            $challanNumbers = [];
            $stmtCh = $pdo->prepare("SELECT DISTINCT `ch_no` FROM `remaining_machines` WHERE `machine_no` IN (SELECT `machine_no` FROM `invoice_machines` WHERE `inv_no` = :inv_no)");
            $stmtCh->execute([':inv_no' => $inv_no]);
            $challanNumbers = $stmtCh->fetchAll(PDO::FETCH_COLUMN);
            $challanNumbersStr = implode(', ', $challanNumbers);

            // 5. Fetch buyer party name
            $partyName = 'CG POWER AND INDUSTRIAL SOLUTION LTD';
            if (!empty($invoice['party_id'])) {
                $stmtP = $pdo->prepare("SELECT `party_name` FROM `parties` WHERE `id` = :party_id");
                $stmtP->execute([':party_id' => $invoice['party_id']]);
                $pRow = $stmtP->fetch();
                if ($pRow) {
...
// Calculate sum of totals across all rows
$total_qty = 0;
$total_sub_total = 0;
$total_cgst = 0;
$total_sgst = 0;
...
if (!empty($invoiceRows)) {
    foreach ($invoiceRows as $row) {
        ...
        $total_net_total += floatval($row['net_total']);
    }
}

$calculated_total = $total_sub_total + $total_cgst + $total_sgst;
$rounding = $total_net_total - $calculated_total;
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Tax Invoice - <?php echo htmlspecialchars($inv_no ?? 'Error'); ?></title>
    <!-- FontAwesome Icons -->
    ...
    <style>
        ...
        @media print {
            ...
            @page {
                size: A4 portrait;
                margin: 0; /* Hides browser default header and footer (URL, timestamp, etc) */
            }
            .invoice-wrapper {
                ...
                margin: 10mm; /* Keep border and content inside printable area */
                box-sizing: border-box;
            }
        }
    </style>
...
<body onload="window.print()">
    ...
    <div class="print-actions">
        ...
        <?php if ($invoice): ?>
            <button class="btn btn-secondary" style="background-color: #f59e0b; color: white; border-color: #d97706;" onclick="window.location.href = 'tax_invoice.php?edit=<?php echo urlencode($invoice['inv_no']); ?>'"><i class="fa-solid fa-pen-to-square"></i> Edit Invoice</button>
        <?php endif; ?>
        <button class="btn btn-secondary" onclick="window.close();">Close Window</button>
        <?php if ($invoice): ?>
            <button class="btn btn-secondary" onclick="window.location.href = 'tax_invoice.php'">Back to Invoices</button>
        <?php endif; ?>
    </div>
    ...
    <?php else: ?>
        ...
        <div class="invoice-wrapper">
            ...
            <div class="invoice-header">
                <div class="logo-box">
                    <div class="sahara-logo-graphics">
                        <div class="logo-s-curve">S</div>
                        <div class="logo-bolt-overlay"><i class="fa-solid fa-bolt"></i></div>
                    </div>
                    <div class="logo-text-group">
                        <div class="logo-text-sahara">SAHARA</div>
                        <div class="logo-text-elec">ELECTRICAL</div>
                    </div>
                </div>
                <div class="header-text">
                    <div class="invoice-title">TAX INVOICE</div>
                    <div class="company-name">SAHARA ELECTRICAL</div>
                    <div class="company-address">M-45 MIDC AHMEDNAGAR - 414111., MAHARASHTRA , STATE CODE : 27</div>
                    <div class="company-tax">GSTIN NO : 27HCFPS4726B1ZI, PAN NO : HCFPS4726B</div>
                </div>
                <!-- QR Code Box -->
            ...
            <div class="invoice-meta-row">
                <div class="meta-col-left">
                    <div class="party-box">
                        ...
                        <div class="party-name"><?php echo htmlspecialchars($partyName); ?></div>
                ...
                <div class="meta-col-right">
                    <table class="meta-data-table">
                        <tr>
                            ...
                            <td style="width: 50%;">: <?php echo htmlspecialchars($invoice['inv_no']); ?></td>
                        </tr>
                        <tr>
                            <td><strong>Invoice Date</strong></td>
                            <td>: <?php echo formatDateDisplay($invoice['inv_date']); ?></td>
                        </tr>
                        <?php if (false): ?>
                        <tr>
                            <td><strong>Part Number (M3)</strong></td>
                            <td style="font-weight: bold; color: #1e3a8a;">: <?php echo htmlspecialchars($invoice['part_no'] ?: '-'); ?></td>
                        </tr>
                        <?php endif; ?>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>: <?php echo htmlspecialchars($invoice['vehicle_no'] ?: '-'); ?></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>: <?php echo formatDateDisplay($invoice['po_date']); ?></td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div style="flex-grow: 1; display: flex; flex-direction: column;">
                ...
                <thead>
                    <tr>
                        <th style="width: 6%;">ITM NO</th>
                        <th style="width: 37%;">Description</th>
                        <th style="width: 19%;">DRG NO.</th>
                ...
                <tbody>
                    <?php
                    // Render up to 6 items (using invoiceRows, padded to 6)
                    for ($i = 0; $i < 6; $i++) {
                        if (isset($invoiceRows[$i])) {
                            $row = $invoiceRows[$i];
                            ?>
                            <tr>
                                <td style="text-align: center; vertical-align: top; border-bottom: 1px solid #000;"><?php echo htmlspecialchars($row['item_sr_no'] ?: '-'); ?></td>
                                <td style="vertical-align: top; border-bottom: 1px solid #000;">
                                    <strong><?php echo htmlspecialchars($row['wound_code']); ?></strong>
                                    <?php if ($row['description'] && trim($row['description']) !== trim($row['wound_code'])): ?>
                                        <br><span style="color: #333; font-size: 10.0px;"><?php echo htmlspecialchars($row['description']); ?></span>
                                    <?php endif; ?>
                                </td>
                                <td style="text-align: center; vertical-align: top; border-bottom: 1px solid #000;"><?php echo htmlspecialchars($row['drg_no'] ?: '-'); ?></td>
                                ...
                                <td style="text-align: right; vertical-align: top; border-bottom: 1px solid #000;"><?php echo number_format($row['rate'], 2); ?></td>
        ...
        <!-- Net Total & Info (Amt in Words + Taxes Table) -->
        <div class="totals-section">
                ...
                <div class="totals-box">
                    <table class="totals-table">
                        ...
                        <?php if (abs($rounding) > 0.001): ?>
                            <tr>
                                <td class="lbl">Rounding</td>
                                <td class="val"><?php echo number_format($rounding, 2); ?></td>
                            </tr>
                        <?php endif; ?>
            ...
            <table class="footer-grid-table">
                <tr>
                    ...
                    <td style="width: 25%; text-align: center; vertical-align: top;">
                        <strong>For SAHARA ELECTRICAL</strong>
                        <div style="height: 40px; display: flex; align-items: center; justify-content: center; margin-top: 5px;">
                            <?php if (($invoice['include_sign'] ?? 1) && file_exists(__DIR__ . '/signature.png')): ?>
                                <img src="signature.png?t=<?php echo time(); ?>" alt="Signature" style="max-height: 38px; mix-blend-mode: multiply;">
                            <?php else: ?>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const invNo = "<?php echo htmlspecialchars($invoice['inv_no']); ?>";
            const invDate = "<?php echo formatDateDisplay($invoice['inv_date']); ?>";
            const qrText = `Invoice No: ${invNo}\nInvoice Date: ${invDate}`;
