<?php
// Sahara Electrical - Centralized Database Configuration (db_config.php)

define('LOGIN_PASSWORD', getenv('LOGIN_PASSWORD') ?: 'sahara@8788'); // Custom password for system access

if (!file_exists(__DIR__ . '/qrcode.min.js')) {
    $src = "C:\\Users\\DELL\\.gemini\\antigravity-ide\\brain\\14bd0e60-4118-4c25-8554-e9ac3b69bd30\\.system_generated\\steps\\188\\content.md";
    if (file_exists($src)) {
        $content = file_get_contents($src);
        $pos = strpos($content, "var QRCode");
        if ($pos !== false) {
            file_put_contents(__DIR__ . '/qrcode.min.js', substr($content, $pos));
        }
    }
}
...
if ($host) {
    ...
    if (
        ...
        $hostLower === '127.0.0.1' ||
        $hostLower === '::1' ||
        strpos($hostLower, '.') === false || // Local hostnames without dots (e.g. sahara, apexx, electrical)
...
if ($isLocalhost) {
    ...
    // -------------------------------------------------------------------------
    define('DB_HOST', getenv('PROD_DB_HOST') ?: 'sql213.infinityfree.com');
    define('DB_USER', getenv('PROD_DB_USER') ?: 'if0_42305887');
    define('DB_PASS', getenv('PROD_DB_PASS') !== false ? getenv('PROD_DB_PASS') : '');
    define('DB_NAME', getenv('PROD_DB_NAME') ?: 'if0_42305887_db');
}
