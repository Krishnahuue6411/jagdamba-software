<?php
// Jagdamba Electrical - Invoice Stock Deductions Ledger (invoice_deductions.php)
            ...
            const res = await App.api('query', {
                sql: `
                    SELECT
                        ...
                        ti.wound_code,
                        ti.party_id,
                        icl_sum.m_code as rm_code,
                        rm.m_description,
                        rm.head as material_type,
                        icl_sum.deducted_qty,
                        rm.unit as material_unit
                    FROM (
                        SELECT
                            TRIM(inv_no) AS inv_no,
                            m_code,
                            SUM(qty_used) AS deducted_qty
                        FROM invoice_consumption_log
                        GROUP BY TRIM(inv_no), m_code
                    ) icl_sum
                    JOIN (
                        SELECT
                            TRIM(inv_no) AS inv_no,
                            MIN(inv_date) AS inv_date,
                            SUM(qty) AS invoice_qty,
                            MIN(uom) AS invoice_uom,
                            GROUP_CONCAT(DISTINCT wound_code ORDER BY wound_code SEPARATOR ', ') AS wound_code,
                            MIN(party_id) AS party_id
                        FROM tax_invoice
                        GROUP BY TRIM(inv_no)
                    ) ti ON icl_sum.inv_no = ti.inv_no
                    LEFT JOIN raw_material rm ON icl_sum.m_code = rm.m_code

                    UNION ALL

                    SELECT
                        TRIM(ti.inv_no) as inv_no,
                        MIN(ti.inv_date) as inv_date,
                        (SELECT SUM(ti2.qty) FROM tax_invoice ti2 WHERE TRIM(ti2.inv_no) = TRIM(ti.inv_no)) as invoice_qty,
                        MIN(ti.uom) as invoice_uom,
                        GROUP_CONCAT(DISTINCT ti.wound_code ORDER BY ti.wound_code SEPARATOR ', ') as wound_code,
                        MIN(ti.party_id) as party_id,
                        b.rm_code,
                        b.m_description,
                        b.material_type,
                        SUM(b.req_qty * ti.qty) as deducted_qty,
                        rm.unit as material_unit
                    FROM tax_invoice ti
                    LEFT JOIN bom b ON ti.wound_code = b.wound_code
                        AND (
                            ti.part_no = b.mac_no
                            OR (
                                (ti.part_no IS NULL OR ti.part_no = '' OR NOT EXISTS (SELECT 1 FROM bom b3 WHERE b3.wound_code = ti.wound_code AND b3.mac_no = ti.part_no))
                                AND b.mac_no = (SELECT b2.mac_no FROM bom b2 WHERE b2.wound_code = ti.wound_code ORDER BY b2.id ASC LIMIT 1)
                            )
                        )
                    LEFT JOIN raw_material rm ON b.rm_code = rm.m_code
                    WHERE TRIM(ti.inv_no) NOT IN (SELECT DISTINCT TRIM(inv_no) FROM invoice_consumption_log)
                    GROUP BY TRIM(ti.inv_no), b.rm_code
                    ORDER BY inv_date DESC, inv_no DESC, rm_code ASC
                `
            });
