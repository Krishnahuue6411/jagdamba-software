-- Jagdamba Electrical - Database Schema Definition
...
CREATE TABLE IF NOT EXISTS `bom` (
  ...
  `bom_tier` ENUM('PRIMARY', 'SECONDARY') NOT NULL DEFAULT 'PRIMARY',
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX (`wound_code`),
  INDEX (`rm_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `tax_invoice` (
  ...
  `party_id` INT NULL,
  `part_no` VARCHAR(50) NULL,
  `include_sign` TINYINT DEFAULT 1,
  `show_asn` TINYINT DEFAULT 0,
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX (`inv_no`),
  INDEX (`wound_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
